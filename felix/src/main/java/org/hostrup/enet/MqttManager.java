    ...
    public void messageArrived(String topic, MqttMessage message) {
        core.getExecutor().submit(() -> {
            try {
                ...
                if (core.debugMode) core.addLog("DEBUG EGRESS: [" + eName + "] (UID: " + uid + ") Received MQTT Command: " + clean);

                ISimpleControl sc = core.getSimpleControl();
                if (sc == null) return;

                // Parse brightness if present in the command
                boolean hasBrightness = clean.contains("brightness:");
                int haBright = 0;
                int enetPct = 0;
                if (hasBrightness) {
                    ...
                    if (bEndIdx > bIdx) {
                        try {
                            haBright = Integer.parseInt(clean.substring(bIdx, bEndIdx));
                            enetPct = (int) Math.round((haBright / 255.0) * 100.0);
                ...
                if (hasBrightness && isDimmable) {
                    ...
                    // If brightness is sent to a non-dimmable light, convert >0 to ON and ==0 to OFF.
                    boolean stateOn = clean.contains("state:ON") || clean.equals("ON") || clean.contains("state:open")
                                     || (hasBrightness && haBright > 0);
                    ...
                    if (stateOn || stateOff) {
                        ...
                        if (baseTopic.contains("/cover/")) {
                            stateVal = stateOn ? "open" : "closed";
                        } else {
                            stateVal = stateOn ? "ON" : "OFF";
                        }
